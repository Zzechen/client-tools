...
export function registerImageTools(server: McpServer): void {
  server.tool(
    ...
    async ({ platform, tag, file, image, ext, timestamp }) => {
      try {
        ...
        const res = await sdkPost(platform, "/inspector/push-image", PushImageRequestSchema, req, PushImageResponseSchema);
  ...
  server.tool(
    ...
    async ({ platform, tag, timestamp }) => {
      try {
        ...
        const res = await sdkPost(platform, "/inspector/show-image", ShowImageRequestSchema, req, ShowImageResponseSchema);
  ...
  server.tool(
    ...
    async ({ platform }) => {
      try {
        const res = await sdkGet(platform, "/inspector/images", ImageListResponseSchema);
