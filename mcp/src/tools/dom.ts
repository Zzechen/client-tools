...
import { z } from "zod";
import { sdkGet, platformParam } from "../sdk-client.js";
...
export function registerDomTools(server: McpServer): void {
  server.tool(
    ...
    async ({ platform }) => {
      try {
        const res = await sdkGet(platform, "/dom/all", DomAllResponseSchema);
  ...
  server.tool(
    ...
    { ...platformParam, id: z.string().describe("DOM 元素的 id 属性值") },
    async ({ platform, id }) => {
      try {
        const res = await sdkGet(platform, `/dom/${encodeURIComponent(id)}`, DomNodeResponseSchema);
